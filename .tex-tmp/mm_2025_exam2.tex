...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Trapezium rule overestimates when the graph is concave up ($f
  \item (x) > 0$). $f(x) = x^3 + 1$ is concave up for $x > 0$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item A – middle column has $x = 0$ and $g
  \item ,
...
\begin{itemize}[nosep]
  ...
  \item (p)(x - p)$, i.e. $y = \\cos(p)(x-p) + \\sin(p) + 1$.
\end{itemize}
