...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Where $f'(x) = 0$, $f$ has stationary points.
  \item Where $f' > 0$, $f$ is increasing; where $f' < 0$, $f$ is decreasing.
  \item The shape of $f'$ determines the concavity and turning points of $f$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $f'(x) = e^{g(x^2)} \cdot g'(x^2) \cdot 2x = 2xg'(x^2)e^{g(x^2)}$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item Under $T$: $(x, y) \to (x', y')$ where $x = 2x' + 4$ and $y = y'$...
...
\begin{itemize}[nosep]
  ...
  \item $\sigma \approx 7.238$. Hmm, but that's option A. Let me re-examine.
  \item Actually that gives $\sigma \approx 7.24$ which is option A, and $\mu = 14.48$.
  \item But checking: answer likely A = 7.238.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item $A'(m) = \frac{9 - 3m^2}{2} = 0 \implies m = \sqrt{3}$.
  \item $A(\sqrt{3}) = \frac{\sqrt{3}(9-3)}{2} = \frac{6\sqrt{3}}{2} = 3\sqrt{3}$.
\end{itemize}
...
\begin{itemize}[nosep]
  \item $f'(x) = -\frac{1}{x+2}$.
  \item Tangent at $x = a$: $y = -\log_e(a+2) - \frac{1}{a+2}(x - a)$.
  \item At $x = 0$: $c = -\log_e(a+2) + \frac{a}{a+2}$.
  \item Maximize $c$ with respect to $a$: $\frac{dc}{da} = -\frac{1}{a+2} + \frac{2}{(a+2)^2} = 0$.
  \item $a + 2 = 2 \implies a = 0$.
  \item $c = -\log_e(2) + 0 = -\log_e(2)$.
  \item Check if this is max: $\frac{d^2c}{da^2} < 0$ at $a = 0$.
  \item Max $c = -\log_e(2)$.
  \item Hmm, but we need to check boundary behavior too. As $a \to -2^+$, $c \to ?$
  \item Answer: $-1 - \log_e(2)$. Need to recheck.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item On $(0, a]$: $h(x) = a/x + b$. As $x \to 0^+$, $h \to \infty$. At $x = a$: $h = 1 + b$. So $(0, a]$ gives $[b+1, \infty)$.
  \item On $[-a, 0)$: $h(x) = a/x + b$. As $x \to 0^-$, $h \to -\infty$. At $x = -a$: $h = -1 + b$. So $[-a, 0)$ gives $(-\infty, b-1]$.
  \item Range $= (-\infty, b-1] \cup [b+1, \infty)$.
  \item This is the same as $[b-1, b+1]^c$ = complement. So range excludes $(b-1, b+1)$.
  \item Answer: $[b-1, b+1]$... no, the range IS $(-\infty, b-1] \cup [b+1, \infty)$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item If $x$ = number of 6's, then $w$ = number of non-6's $= 20 - x$.
  \item $q(w) = p(20 - w)$.
  \item Answer: A.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item Range of $g = [-1, 0]$: $\log_2(\cos(2\pi x)) \in [-1, 0]$.
  \item So $\cos(2\pi x) \in [2^{-1}, 2^0] = [1/2, 1]$.
  \item $\cos(2\pi x) \ge 1/2$ when $2\pi x \in [-\pi/3 + 2k\pi, \pi/3 + 2k\pi]$.
  \item i.e. $x \in [-1/6 + k, 1/6 + k]$.
  \item For $k = 0$: $x \in [-1/6, 1/6]$. Hmm, but need to match options.
  \item A possible interval: $[1/4, 5/12]$... check option A.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $(x+2)^2(x-2)^2 = [(x+2)(x-2)]^2 = (x^2-4)^2 = x^4 - 8x^2 + 16$.
  \item $f(x) = \frac{1}{4}(x^4 - 8x^2 + 16) = \frac{1}{4}x^4 - 2x^2 + 4$.
  \item So $b = -2$, $c = 4$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $f''(x) = 3x^2 - 4 = 0 \implies x = \frac{2}{\sqrt{3}} = \frac{2\sqrt{3}}{3}$.
  \item $f'\left(\frac{2\sqrt{3}}{3}\right) = \left(\frac{2\sqrt{3}}{3}\right)^3 - 4\left(\frac{2\sqrt{3}}{3}\right) = \frac{8 \cdot 3\sqrt{3}}{27} - \frac{8\sqrt{3}}{3} = \frac{8\sqrt{3}}{9} - \frac{8\sqrt{3}}{3} = -\frac{16\sqrt{3}}{9}$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item The 'no swimming' zone is the river region below $y = 30$.
...
\begin{itemize}[nosep]
  ...
  \item Wait that doesn't seem right for the options. Let me re-read: 'no later than 3 min after' = $T \le 3$ given $T > 0$.
  \item Answer $\approx 0.547$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item The new interval $[-4.5, 0.5]$ has midpoint $-2$. For symmetry about $k$:
  \item If $k = -2$: $\Pr(-4.5 \le T \le 0.5) = \Pr(-2.5/4 \le Z \le 2.5/4) = \Pr(-0.625 \le Z \le 0.625)$.
  \item Use CAS to find values of $k$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $f'(x) = 2e^{1-x^2} + 2x \cdot (-2x)e^{1-x^2} = 2e^{1-x^2}(1 - 2x^2)$.
  \item $f'(1) = 2e^0(1-2) = 2(-1) = -2$.
  \item Hmm wait: $f'(1) = 2e^{1-1}(1-2) = 2(1)(-1) = -2$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $f'(x) = 2e^{1-x^2}(1 - 2x^2)$.
  \item $f'(p) = 2(1 - 2p^2)e^{1-p^2}$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Slope at $x = 1$: $f'(1) = -2$.
  \item For perpendicular: $f'(p) \times (-2) = -1 \implies f'(p) = 1/2$.
  \item $2(1-2p^2)e^{1-p^2} = 1/2$.
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Tangent at $x = 1$: $y - f(1) = f'(1)(x - 1)$, i.e. $y - 2 = -2(x-1) \implies y = -2x + 4$.
  \item Tangent at $x = p$: $y - f(p) = f'(p)(x - p)$ with $f'(p) = 1/2$.
  \item Solve simultaneously using CAS.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $f'(x) = 3x^2 - 1$. Tangent at $x = a$:
  \item $y - f(a) = f'(a)(x - a)$.
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item When $3a^2 - 1 = 0$, the slope $f'(a) = 0$, so the tangent is horizontal.
  \item A horizontal tangent has no $x$-intercept (unless it lies on the $x$-axis).
  \item At $a = 1/\sqrt{3}$: $f(a) = \frac{1}{3\sqrt{3}} - \frac{1}{\sqrt{3}} = -\frac{2}{3\sqrt{3}} \ne 0$. So no $x$-intercept.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Parallel means same slope: $f'(a) = f'(b)$, i.e. $3a^2 - 1 = 3b^2 - 1$, so $a^2 = b^2$.
  \item Since $b \ne a$: $b = -a$.
  ...
  \item So we need $f'(a) = f'(b)$ where $b = 2a^3/(3a^2-1)$.
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Tangent at $(t, p(t))$: $y = p'(t)(x - t) + p(t) = (3t^2 + w)(x - t) + t^3 + wt$.
  \item At $x = -t$: $0 = (3t^2 + w)(-2t) + t^3 + wt$.
  \item $0 = -6t^3 - 2wt + t^3 + wt = -5t^3 - wt$.
  \item $t(-5t^2 - w) = 0$. Since $t > 0$: $w = -5t^2$.
  \item This must hold for some specific $t > 0$, so $w < 0$.
  \item Wait, the question says 'for some $t > 0$', so any $w < 0$ works. But let me re-read.
  \item Actually, the tangent at $(t, p(t))$ should have $x$-intercept at $(-t, 0)$. So $b = -t$ where $b = \frac{2t^3}{3t^2 + w}$ (analogous to part a with $f$ replaced by $p$).
  \item $-t = \frac{2t^3}{3t^2 + w} \implies -t(3t^2+w) = 2t^3 \implies -3t^3 - wt = 2t^3 \implies w = -5t^2$.
  \item For this to hold for some $t > 0$: $w$ can be any negative value. Hmm.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item Under $T$: $x' = mx + h$, $y' = ny + k$.
  \item For parallel tangents at $x' = -t$ and $x' = t$: the symmetry about $x' = 0$ requires $h = 0$.
  \item No restrictions on $m$, $n$, or $k$.
\end{itemize}
