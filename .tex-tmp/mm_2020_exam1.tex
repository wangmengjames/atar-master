...
\begin{itemize}[nosep]
  ...
  \item Check: $6^3 = 216$, $5^4 - 2^4 = 625 - 16 = 609$. ✓
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item For the tangent at $Q(a, f(a))$ to pass through $P(1, 0)$, the slope PQ must equal $f
  \item ,
...
\begin{itemize}[nosep]
  ...
  \item (x) = 0$: $\\log_e(x) = -1 \\implies x = e^{-1} = \\frac{1}{e}$.
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $g(x) = x\\log_e(x) + k$, $g
  \item ,
                
  \item (x_0) = 2 \\implies \\log_e(x_0) = 1 \\implies x_0 = e$.
...
\begin{itemize}[nosep]
  ...
  \item Let $h(x) = x(1 - \\log_e(x))$. Maximum of $h$: $h
  \item ,
