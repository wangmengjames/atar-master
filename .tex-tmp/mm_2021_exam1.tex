...
\textbullet{} dilation by a factor of $\frac{1}{2}$ from the vertical axis (parallel to the horizontal axis)
\textbullet{} translation by two units to the right (in the direction of the positive horizontal axis)
...
\textbullet{} glazed, with custard
\textbullet{} glazed, with no custard
\textbullet{} not glazed, with custard
\textbullet{} not glazed, with no custard.

It is known that, in the box:
\textbullet{} $\frac{1}{2}$ of the doughnuts are with custard
\textbullet{} $\frac{7}{10}$ of the doughnuts are not glazed
\textbullet{} $\frac{1}{10}$ of the doughnuts are glazed, with custard.
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Product rule: $f'(x) = \sqrt{2x+1} + x \cdot \frac{1}{\sqrt{2x+1}}$.
  \item Simplify: $f'(x) = \sqrt{2x+1} + \frac{x}{\sqrt{2x+1}} = \frac{2x+1+x}{\sqrt{2x+1}} = \frac{3x+1}{\sqrt{2x+1}}$.
  \item Evaluate: $f'(4) = \frac{13}{\sqrt{9}} = \frac{13}{3}$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Range of $\sin$ is $[-1,1]$, so range of $2\sin(2x)$ is $[-2, 2]$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item $k[-\frac{1}{x}]_1^2 = k(-\frac{1}{2} + 1) = \frac{k}{2} = 1$.
  \item $k = 2$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item $= 2[\log_e(x)]_1^2 = 2(\log_e 2 - 0) = 2\log_e(2)$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item Alternative: test sign of $f'$ either side of $x=3$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $O = (0,0)$, $A = (2,0)$, $P' = (\cos\theta, \sin\theta)$ on the unit circle.
  \item Area of triangle $OAP' = \frac{1}{2} |\text{base}| \times |\text{height}|$.
  \item Base $OA = 2$ along x-axis. Height = $|\sin\theta|$ (perpendicular distance from $P'$ to x-axis).
  \item $g(\theta) = \frac{1}{2} \cdot 2 \cdot |\sin\theta| = |\sin\theta|$.
  \item Domain depends on the constraint $0 < q \le 1$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item This occurs when $\theta = \pi/2$, i.e. $P' = (0, 1)$.
  \item Maximum area $= 1$ square unit.
\end{itemize}
