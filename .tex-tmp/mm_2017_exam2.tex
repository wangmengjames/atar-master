...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Period $= \frac{2\pi}{2} = \pi$. Range: $5(-1) - 1 = -6$ to $5(1) - 1 = 4$, so $[-6, 4]$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $f'(x) < 0$ between the stationary points, i.e. $\left(-3, \frac{5}{3}\right)$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $f'(x) = 3x^2 + 2ax + b = 0$ at $x = -1$ and $x = 3$. Sum of roots $= -1 + 3 = 2 = -\frac{2a}{3}$, so $a = -3$. Product $= -1 \times 3 = -3 = \frac{b}{3}$, so $b = -9$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $\sin(2x) = \frac{\sqrt{3}}{2}$ gives $2x = \frac{\pi}{3} + 2k\pi$ or $2x = \pi - \frac{\pi}{3} + 2k\pi = \frac{2\pi}{3} + 2k\pi$. So $x = \frac{\pi}{6} + k\pi$ or $x = \frac{\pi}{3} + k\pi$. In $[-\pi, 0]$: $x = \frac{\pi}{6} - \pi = -\frac{5\pi}{6}$ and $x = \frac{\pi}{3} - \pi = -\frac{2\pi}{3}$. Sum so far $= -\frac{3\pi}{2}$. Need total sum $= -\pi$, so remaining solutions must add to $\frac{\pi}{2}$. Next solutions: $\frac{\pi}{6}$ and $\frac{\pi}{3}$. $\frac{\pi}{6} + \frac{\pi}{3} = \frac{\pi}{2}$. \checkmark{} So $d$ just needs to include both, $d \ge \frac{\pi}{3}$. But must not include next solution at $\frac{\pi}{6} + \pi = \frac{7\pi}{6}$. Answer: $d = 0$ doesn't work. Checking: if $d = 0$, no solutions in $(0, 0]$, sum $= -\frac{3\pi}{2} \ne -\pi$. So need $d$ past $\frac{\pi}{3}$. Among the options, $d = 0$ is option A... Let me reconsider. Maybe I need to be more careful about which solutions are in $[-\pi, d]$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Check each: A. $h(x)h(-x) = \frac{1}{x-1} \cdot \frac{1}{-x-1} = \frac{1}{-(x-1)(x+1)} = \frac{-1}{x^2-1} = -h(x^2)$? $h(x^2) = \frac{1}{x^2-1}$. So $h(x)h(-x) = \frac{-1}{x^2-1} = -h(x^2)$. A is true. B. $h(x) + h(-x) = \frac{1}{x-1} + \frac{1}{-x-1} = \frac{-x-1+x-1}{(x-1)(-x-1)} = \frac{-2}{-(x^2-1)} = \frac{2}{x^2-1} = 2h(x^2)$. But B says $= 2h(x^2)$... that's true too. Hmm. Let me check all more carefully.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $E(X) = -p + 0 + (1-3p) = 1 - 4p$. $E(X^2) = p + 0 + (1-3p) = 1 - 2p$. $\text{Var}(X) = E(X^2) - (E(X))^2 = (1-2p) - (1-4p)^2 = 1-2p - 1 + 8p - 16p^2 = 6p - 16p^2$. Hmm, checking options: B is $1-4p$. Let me recheck. $E(X^2) = (-1)^2 p + 0^2(2p) + 1^2(1-3p) = p + 1 - 3p = 1 - 2p$. $E(X) = 1 - 4p$. $\text{Var} = 1-2p - (1-4p)^2 = 1 - 2p - 1 + 8p - 16p^2 = 6p - 16p^2 = 2p(3 - 8p)$. None of the options match perfectly... Let me check option B: $1 - 4p$. That doesn't match. Actually I should recheck more carefully.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Area $= u(-u^3 + 8) = -u^4 + 8u$. $\frac{dA}{du} = -4u^3 + 8 = 0$, $u^3 = 2$, $u = \sqrt[3]{2}$. $A = -(\sqrt[3]{2})^4 + 8\sqrt[3]{2} = -2^{4/3} + 8 \cdot 2^{1/3} = 2^{1/3}(8 - 2) = 6\sqrt[3]{2}$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Since $f(-x) = f(x)$, the function is even. The graph is symmetric about the $y$-axis. Above x-axis on $[a, b]$ and $[c, d]$, below on $[b, c]$. Area $= \int_a^b f(x)\,dx - \int_b^c f(x)\,dx + \int_c^d f(x)\,dx$. By symmetry, $= 2\int_a^b f(x)\,dx + \int_b^c f(x)\,dx$... Using the symmetry property and the options given.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $\int_k^{k+1} (\cos(x) + 1)\,dx = [\sin(x) + x]_k^{k+1} = \sin(k+1) + k + 1 - \sin(k) - k = \sin(k+1) - \sin(k) + 1 = 1$. So $\sin(k+1) = \sin(k)$. This means $k+1 = \pi - k$, i.e. $2k = \pi - 1$, $k = \frac{\pi - 1}{2} \approx 1.07$. Or $k + 1 = k + 2n\pi$ (not useful). Checking $\frac{\pi-1}{2} \approx 1.07$, and $0 < k < 2$. Among options, $\pi - 1 \approx 2.14$... $\frac{3\pi}{4} \approx 2.36$... Hmm. Let me check option C: $k = \frac{3\pi}{4}$. But $\frac{3\pi}{4} \approx 2.36 > 2$... The constraint says $0 < k < 2$. Option C is $\frac{3\pi}{4} \approx 2.36$. Hmm.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $f'(x) = 3x^2 - 5 = 0 \implies x = \pm\sqrt{\frac{5}{3}} = \pm\frac{\sqrt{15}}{3}$.
  \item $f\left(\frac{\sqrt{15}}{3}\right) = \frac{5\sqrt{15}}{9} - \frac{5\sqrt{15}}{3} = -\frac{10\sqrt{15}}{9}$.
  \item Turning points: $\left(-\frac{\sqrt{15}}{3}, \frac{10\sqrt{15}}{9}\right)$ (local max) and $\left(\frac{\sqrt{15}}{3}, -\frac{10\sqrt{15}}{9}\right)$ (local min).
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item For $k = 1$: $CD = 2$, $k+1 = 2$. \checkmark{}. For $k = \frac{7}{3}$: $CD = 2\sqrt{1 + \frac{16}{9}} = 2\sqrt{\frac{25}{9}} = \frac{10}{3}$, $k+1 = \frac{10}{3}$. \checkmark{}.
...
\begin{itemize}[nosep]
  ...
  \item $= \left[\frac{(k+1)x^2}{2} - \frac{x^4}{4}\right]_0^{\sqrt{k+1}} = \frac{(k+1)^2}{2} - \frac{(k+1)^2}{4} = \frac{(k+1)^2}{4}$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item First: $\frac{1}{625}[\frac{(t-20)^2}{2}]_{25}^{45} = \frac{1}{1250}(625 - 25) = \frac{600}{1250} = \frac{12}{25}$.
  \item Second: $\frac{1}{625}[70t - \frac{t^2}{2}]_{45}^{55} = \frac{1}{625}[(3850 - 1512.5) - (3150 - 1012.5)] = \frac{1}{625}[2337.5 - 2137.5] = \frac{200}{625} = \frac{8}{25}$.
  \item Total: $\frac{12}{25} + \frac{8}{25} = \frac{20}{25} = \frac{4}{5}$.
  \item Wait, let me redo. First integral: $\frac{1}{625} \cdot \frac{(t-20)^2}{2}\Big|_{25}^{45} = \frac{1}{1250}(25^2 - 5^2) = \frac{1}{1250}(625-25) = \frac{600}{1250} = \frac{12}{25}$.
  \item Second integral: $\frac{1}{625}\left[70t - \frac{t^2}{2}\right]_{45}^{55} = \frac{1}{625}\left[(3850-1512.5)-(3150-1012.5)\right] = \frac{1}{625}(2337.5-2137.5) = \frac{200}{625} = \frac{8}{25}$.
  \item Total = $\frac{20}{25} = \frac{4}{5}$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Need to find $a$ in $[20, 45]$ (since $\Pr(T \ge 45) = 0.5$ and we need 0.7 > 0.5).
  \item $\Pr(T \ge a) = 1 - \Pr(T < a) = 1 - \int_{20}^{a} \frac{1}{625}(t-20)\,dt = 1 - \frac{(a-20)^2}{1250} = 0.7$.
  \item $\frac{(a-20)^2}{1250} = 0.3$, $(a-20)^2 = 375$, $a - 20 = \sqrt{375} = 5\sqrt{15}$.
  \item $a = 20 + 5\sqrt{15} \approx 39.3649$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item $= 7p^2(1-p)^4[3(1-p) + 5p] = 7p^2(1-p)^4(3 + 2p)$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $f'(x) = 2^{x+1} \ln(2)$. $f'(0) = 2\ln(2)$.
  \item $(f^{-1})'(x) = \frac{1}{(x+2)\ln(2)}$. $(f^{-1})'(0) = \frac{1}{2\ln(2)}$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Gradient of $g_k$ at $x = 0$: $g_k'(0) = 2k$.
  \item Gradient of $g_k^{-1}$ at $x = 0$: $(g_k^{-1})'(0) = \frac{1}{2k}$.
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $g_k(x) = g_k^{-1}(x)$ where both equal $x$ (since they're inverse functions, solutions lie on $y = x$).
  \item So $g_k(x) = x$: $2e^{kx} - 2 = x$.
  \item For exactly one solution, the line $y = x + 2$ is tangent to $y = 2e^{kx}$.
  \item At tangent point: $2ke^{kx} = 1$ (gradient = 1) and $2e^{kx} = x + 2$.
  \item From first: $e^{kx} = \frac{1}{2k}$. Substitute: $2 \cdot \frac{1}{2k} = x + 2$, $\frac{1}{k} = x + 2$.
  \item Also $kx = \ln(\frac{1}{2k}) = -\ln(2k)$, $x = \frac{-\ln(2k)}{k}$.
  \item So $\frac{-\ln(2k)}{k} + 2 = \frac{1}{k}$, $-\ln(2k) + 2k = 1$, $\ln(2k) = 2k - 1$.
  \item By inspection: $k = \frac{1}{2}$ gives $\ln(1) = 0$ and $2(\frac{1}{2}) - 1 = 0$. \checkmark{}.
  \item $p = \frac{1}{2}$.
\end{itemize}
