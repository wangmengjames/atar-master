...
\begin{itemize}[nosep]
  ...
  \item $pn < qm$ is the statement in C, so C says it
  \item ,
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item We need a transformation that preserves the median at $x=0$, maps a pdf to a pdf, and changes $f
  \item (0)=-1$.
  \item If $T$ is a dilation: $g(x) = \\frac{1}{|a|}f(x/a)$, then $g
  \item (x/a)$.
  \item $g
  \item (0) = \\frac{4}{a|a|}$. Need $\\frac{4}{a|a|} = -1$, so $a|a| = -4$, $a = -2$ (then $a|a| = -2 \\cdot 2 = -4$).
...
\begin{itemize}[nosep]
  ...
  \item Wait: also $b < -5$ won
  \item s a local min at $x=1$ with $f(1)=-5$.
...
\begin{itemize}[nosep]
  ...
  \item (-\\frac{1}{3}) = 12(-\\frac{1}{27}) + 12(\\frac{1}{9}) - 24(-\\frac{1}{3}) = -\\frac{4}{9} + \\frac{4}{3} + 8 = -\\frac{4}{9} + \\frac{12}{9} + \\frac{72}{9} = \\frac{80}{9}$.
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item From the factored form of $p
  \item ,
