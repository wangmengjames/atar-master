...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $y = x^{4/3}$: passes through origin and $y
\end{itemize}
...
\begin{itemize}[nosep]
  \item $h
  \item (x) = (\\ln 2)^2 \\cdot 2^x - 2 = 0$. Solve using CAS.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $\\log_e(2) \\cdot 2^x - 2x = 0$ is $h
  \item s formula.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item M1: Local min on $x$-axis means $f(a) = 0$ and $f
  \item ,
