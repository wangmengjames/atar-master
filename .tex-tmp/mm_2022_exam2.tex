...
\textbullet{} The average rate of change of $g$ between $x = a$ and $x = b$ is positive.
\textbullet{} The instantaneous rate of change of $g$ at $x = \frac{a+b}{2}$ is negative.
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $\tan\left(\frac{x}{3}\right)$ has a vertical asymptote at $x = \frac{3\pi}{2} \approx 4.71$, which is in $[0, 5]$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Vertex at $x = -\frac{3}{2} = -1.5$. For one-to-one on $(-\infty, a]$, need $a \le -1.5$. Largest value is $a = -1.5$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item The original function has a local min (left) and local max (right), so $f'(x) = 0$ at two points, positive between them. Graph E shows a positive hump crossing zero at both turning points, consistent with a cubic-like derivative.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Need $\frac{x+a}{x-a} > 0$. Both factors same sign: $x > a$ or $x < -a$. Domain is $R \setminus [-a, a]$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Need $x^2 - 2x - 3 \ge 0$, i.e. $(x-3)(x+1) \ge 0$. Solution: $x \le -1$ or $x \ge 3$. Domain: $(-\infty, -1] \cup [3, \infty)$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $f'(x) = x^2 + 2mx + n$. Turning points at $x = -3$ and $x = 1$: $f'(x) = (x+3)(x-1) = x^2 + 2x - 3$.
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $g(b) > g(a)$ (positive average rate), but $g'\left(\frac{a+b}{2}\right) < 0$ (decreasing at midpoint). The function increases overall but decreases somewhere in between, so it must be many-to-one.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $V = x(a-2x)(b-2x)$. $V' = 12x^2 - 4(a+b)x + ab = 0$.
  \item $x = \frac{4(a+b) \pm \sqrt{16(a+b)^2 - 48ab}}{24} = \frac{a+b \pm \sqrt{a^2 - ab + b^2}}{6}$.
  \item Maximum at the smaller root: $x = \frac{a+b - \sqrt{a^2 - ab + b^2}}{6}$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item M1: Find x-coordinate: $f'(x) = -2 \Rightarrow \frac{x}{6} = -2 \Rightarrow x = -12$.
  \item A1: $f(-12) = \frac{144}{12} = 12$. Tangent: $y - 12 = -2(x + 12) \Rightarrow y = -2x - 12$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item M1: $g'(x) = \frac{x}{2a^2}$. At $x = -b$: gradient $= -\frac{b}{2a^2}$, perpendicular gradient $= \frac{2a^2}{b}$.
  \item M1: Find intersection of perpendicular line with $g(x)$, set up area integral.
  \item M1: Express area as a function of $b$ and differentiate.
  \item A1: $b = a\sqrt[3]{2}$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item M1: From graph: min fox pop = 700, max = 2500. Mean $= \frac{700+2500}{2} = 1600$ \checkmark{}. Amplitude $a = 2500 - 1600 = 900$.
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item M1: Find the first two maximum points of $s(t)$ using CAS (e.g., $t_1 \approx 40$, $t_2 \approx 200$, or solve $s'(t) = 0$).
  \item A1: Average rate of change $= \frac{s(t_2) - s(t_1)}{t_2 - t_1} \approx -3.0$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item M1: Find $s'(t)$ and maximise it for $t > 40$ using CAS.
  \item A1: $t \approx 156$ weeks.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item A1: $f'(x) = \frac{1}{\frac{1}{2}+x} + \frac{1}{\frac{1}{2}-x}$. At $x = 0$: $f'(0) = 2 + 2 = 4$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $f'(x) > 0$ for all $x \in \left(-\frac{1}{2}, \frac{1}{2}\right)$ (the entire domain). So $f$ is strictly increasing on its whole domain.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item $g'\left(\frac{\pi}{6}\right) = 2\cos\left(\frac{\pi}{3}\right) \cdot f'\left(\sin\left(\frac{\pi}{3}\right)\right) = 2 \times \frac{1}{2} \times f'\left(\frac{\sqrt{3}}{2}\right) = 1 \times \frac{1}{9} = \frac{1}{9}$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item M1: Average value of $g'(x) = \frac{1}{\frac{\pi}{6}-\frac{\pi}{8}}\int_{\pi/8}^{\pi/6} g'(x)\,dx = \frac{g(\pi/6) - g(\pi/8)}{\frac{\pi}{6}-\frac{\pi}{8}}$.
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item M1: $g'(x) = 2\cos(2x) \cdot f'(\sin(2x)) = 0$. Either $\cos(2x) = 0$ or $f'(\sin(2x)) = 0$.
  \item M1: $\cos(2x) = 0 \Rightarrow 2x = \frac{\pi}{2}, \frac{3\pi}{2} \Rightarrow x = \frac{\pi}{4}, \frac{3\pi}{4}$. (Also $2x = \frac{5\pi}{2}$ gives $x = \frac{5\pi}{4} > \pi$.)
  \item A1: $f'(\sin(2x)) = 0 \Rightarrow \sin(2x) = \frac{\sqrt{2}}{2}$ (from table). $2x = \frac{\pi}{4}, \frac{3\pi}{4}, \frac{9\pi}{4}, \frac{11\pi}{4}, ...$. So $x = \frac{\pi}{8}, \frac{3\pi}{8}, \frac{9\pi}{8}$ (last one $> \pi$). Also $2x = 2\pi + \frac{\pi}{4}$ etc. Four solutions in $[0, \pi]$: $\frac{\pi}{8}, \frac{\pi}{4}, \frac{3\pi}{8}, \frac{3\pi}{4}$.
\end{itemize}
