...
\begin{itemize}[nosep]
  \item $f
  \item infinitely many solutions to $f(x)=4$
  \item ,
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item If $A$ and $B$ are independent, $\\Pr(B|A) = \\Pr(B|A
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item But need to check if all three are in range. Since $0 < \\alpha < \\pi/2$: $x_2 = \\alpha/2 + \\pi < \\pi/4 + \\pi = 5\\pi/4$. ✓
...
\begin{itemize}[nosep]
  ...
  \item  means $y
  \item  < 0$, so $x \\in [0, 20)$.
  \item Hmm, let me reconsider. The gradient function is $y
  \item ,
...
\begin{itemize}[nosep]
  \item $f
  \item (1) = \\frac{1}{f
  \item (0)} = \\frac{1}{0}$, undefined.
  \item At $x = 1$: $f(1) = 0$, tangent to $f$: $y = -3(x-1)$, slope $= -3$.
  \item $f^{-1}(1) = 0$, tangent to $f^{-1}$: slope $= 1/f
  \item ,
                
  \item ,
                
  \item (x) = -\\frac{1}{3(1-x)^{2/3}}$. At $x = 1$: undefined (vertical tangent).
  \item No — $f^{-1}(x) = (1-x)^{1/3}$, $(f^{-1})
  \item ,
