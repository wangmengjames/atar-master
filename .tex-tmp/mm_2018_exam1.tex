...
\begin{itemize}[nosep]
  ...
  \item (\\pi) = \\frac{e^\\pi(\\cos\\pi + \\sin\\pi)}{\\cos^2\\pi} = \\frac{e^\\pi(-1 + 0)}{1} = -e^\\pi$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item Hmm, but the point is $(-5\\pi/2, 5\\pi/2)$. Check: $y = x\\sin(x)$ at $x = -5\\pi/2$: $y = (-5\\pi/2)\\sin(-5\\pi/2) = (-5\\pi/2)(-1) = 5\\pi/2$. ✓
  \item Tangent: $y = -x$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item Try $a = 3\\pi$: $(x - 3\\pi)\\sin(x - 3\\pi) = (x-3\\pi)(-\\sin(x)) = (3\\pi - x)\\sin(x)$. ✓
  \item $a = 3\\pi$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item The tangent $l_1$ at $(\\pi/2, 5\\pi/2)$: $f
  \item ,
                
  \item (\\pi/2) = -1 + 0 = -1$. So $l_1$: $y - \\frac{5\\pi}{2} = -1(x - \\frac{\\pi}{2})$, i.e. $y = -x + 3\\pi$.
