...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Quotient rule: $f'(x) = \frac{(x+2)(1) - x(1)}{(x+2)^2} = \frac{2}{(x+2)^2}$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Chain rule: $g'(x) = 3(2 - x^3)^2 \cdot (-3x^2) = -9x^2(2 - x^3)^2$.
  \item $g'(1) = -9(1)^2(2 - 1)^2 = -9$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item Since $\frac{d}{dx}[x\log_e(3x)] = \log_e(3x) + 1$,
  \item $\int_1^2 (\log_e(3x) + 1)\, dx = [x\log_e(3x)]_1^2 = 2\log_e(6) - \log_e(3) = \log_e(36) - \log_e(3) = \log_e(12)$.
  \item Wait, recheck: $2\log_e(6) - 1 \cdot \log_e(3) = \log_e(36) - \log_e(3) = \log_e(12)$.
  \item Answer: $\log_e(12)$.
\end{itemize}
...
\textit{Marking guide:}
\begin{itemize}[nosep]
  \item x-intercepts: $(x+2)^2(x-1) = 0$ gives $x = -2$ (touch) and $x = 1$. But domain is $[-3, 0]$, so $x = 1$ is excluded.
  \item Only x-intercept in domain: $(-2, 0)$.
  \item y-intercept: $f(0) = (2)^2(-1) = -4$, so $(0, -4)$.
  \item $f'(x) = 3x^2 + 6x = 3x(x+2) = 0$ gives $x = 0$ and $x = -2$.
  \item Stationary points: $(-2, 0)$ (local max) and $(0, -4)$ (endpoint/local min).
  \item Endpoints: $f(-3) = (-1)^2(-4) = -4$.
  \item Graph: starts at $(-3, -4)$, rises to $(-2, 0)$, then falls to $(0, -4)$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item If $c \le -2$, then $g$ is decreasing on $(-\infty, c]$, minimum is $g(c)$.
  \item Wait: for $c < 0$, the function on $(-\infty, c]$ has range $[g(c), \infty)$ if $c \le -2$, or range $[-1, \infty)$ if $-2 < c < 0$... no.
  \item Actually if $c \le -2$: $g$ is decreasing on the domain, so range is $[g(c), \infty)$. Need $g(c) \ge 0$. $g(c) = c^2 + 4c + 3 = (c+1)(c+3) \ge 0$. Since $c \le -2 < -1$, we need $c \le -3$. Largest is $c = -3$.
  \item If $-2 < c < 0$: minimum of $g$ is $g(-2) = -1 < 0$, so range includes negative values. Not valid.
  \item Answer: $c = -3$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item $\Pr(A' \cap B') = 1 - \Pr(A \cup B) = 1 - 8p$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item $= \left[\frac{2}{3}x^{3/2} - \frac{2}{5}x^{5/2}\right]_0^1 = \frac{2}{3} - \frac{2}{5} = \frac{4}{15}$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item $f'(x) = \frac{1}{2}x^{-1/2} - \frac{3}{2}x^{1/2} = \frac{1}{2\sqrt{x}} - \frac{3\sqrt{x}}{2} = \frac{1 - 3x}{2\sqrt{x}}$.
\end{itemize}
...
\begin{itemize}[nosep]
  ...
  \item Set $f'(x) = 1$: $\frac{1-3x}{2\sqrt{x}} = 1 \implies 1 - 3x = 2\sqrt{x}$.
  \item Let $u = \sqrt{x}$: $1 - 3u^2 = 2u \implies 3u^2 + 2u - 1 = 0 \implies (3u - 1)(u + 1) = 0$.
  \item $u = \frac{1}{3}$, so $x = \frac{1}{9}$. Point on curve: $(\frac{1}{9}, f(\frac{1}{9})) = (\frac{1}{9}, \frac{1}{3} \cdot \frac{8}{9}) = (\frac{1}{9}, \frac{8}{27})$.
  \item Since $ABC$ is right-angled and $BC$ is also tangent, gradient of $BC = -1$ (perpendicular tangent).
  \item Set $f'(x) = -1$: $\frac{1-3x}{2\sqrt{x}} = -1 \implies 1 - 3x = -2\sqrt{x} \implies 3x - 2\sqrt{x} - 1 = 0$.
